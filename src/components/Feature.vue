...
const artworks = useCollection(query(collection(firestore, 'artworks'), orderBy('featured')))
...
/**
 * Sets an artwork as featured by its ID.
 * Featured artworks show up on the home page.
 *
 * @param {string} id - The ID of the artwork to be featured
 */
function featureArtwork(id) {
...
/**
 * Removes the featured status from an artwork by its ID.
 * This artwork will no longer show up on the homepage.
 *
 * @param {string} id - The ID of the artwork to be unfeatured
 */
function unfeatureArtwork(id) {
  updateDoc(doc(firestore, "artworks", id), {
    featured: false
  });
}
...
<template>
  <div v-for="artwork in artworks" :key="artwork.id" class="cards">
    <section class="container is-visible">
      ...
      <div class="grid-2-btns">
        <button v-if="!artwork.featured" @click="featureArtwork(artwork.id)">Feature</button>
        <button v-if="artwork.featured" @click="unfeatureArtwork(artwork.id)">Remove</button>
